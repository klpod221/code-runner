...
const configureLimiter = async () => {
  try {
    const windowMins = await settingsService.getNumericSetting(
      "RATE_LIMIT_WINDOW",
      15
    );
    const maxRequests = await settingsService.getNumericSetting(
      "RATE_LIMIT_MAX",
      100
    );
...
// static files serving (landing page)
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`API Documentation available at ${DOMAIN}/docs`);
